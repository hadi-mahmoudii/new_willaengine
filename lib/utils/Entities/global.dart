...
import '../configs/app_session.dart';
...
class GlobalEntity {
  Future<bool> addFiles(ClientDetailsModel user, String batchId) async {
    ...
    var response = await http.put(
      ...
      headers: {'Authorization': AppSession.token, 'Module': 'crm', 'origin': Urls.domain, 'X-Requested-With': 'XMLHttpRequest'},
  ...
  Future<bool> taskAddFiles(ClientTaskOverviewModel task, String batchId) async {
    // final batchId = randomString(50);
    // for (var file in files) {
    //   await addImage(
    //     batchId,
  ...
  Future<MediaObject?> addImage(String batchId, dynamic file) async {
    ...
    var dio = new di.Dio();
    dio.options.headers = {'Authorization': AppSession.token, 'Module': 'crm', 'origin': Urls.domain, 'X-Requested-With': 'XMLHttpRequest'};
    di.FormData formData = new di.FormData.fromMap({"batch_id": batchId, "file": di.MultipartFile.fromBytes(imageListInt, filename: 'test.jpg')});
  ...
  Future<List<Map<String, Object>>> downloadMedias(List<MediaModel> medias) async {
  ...
  String dataFilter(String data, {String replacement = ''}) {
    if (data == 'null' || data == 'NULL' || data == '[]' || data == '{}' || data == null) {
      return replacement;
    } else {
      return data;
    }
  ...
  String dateConvert(String date) {
    try {
      ...
      String dateValue = '${pdate.weekdayname} ${pdate.day} ${pdate.monthname} ${pdate.year} ';
